...
| Standard Template Library (STL) | DotNetDupe |
| :--- | :--- |
| `std::unordered_map<std::string, int> m;`<br>`auto it = m.find("key");`<br>`if (it != m.end()) { int val = it->second; }` | `Dictionary<String, int> dict;`<br>`int val;`<br>`if (dict.TryGetValue(_T("key"), val)) { ... }` |

### 4. Path Handling 📂
| Standard Template Library (STL) | DotNetDupe |
| :--- | :--- |
| `std::filesystem::path p = "dir";`<br>`p /= "file.txt";`<br>`p.replace_extension(".bak");`<br>`std::string s = p.string();` | `String p = Path::Combine({_T("dir"), _T("file.txt")});`<br>`p = Path::ChangeExtension(p, _T(".bak"));` |

### 5. Console & I/O 🖥️
| Standard Template Library (STL) | DotNetDupe |
| :--- | :--- |
| `// Manual color escape codes`<br>`std::cout << "\033[1;32mGreen\033[0m" << std::endl;`<br>`// No standard title or clear API` | `Console::SetForegroundColor(ConsoleColor::Green);`<br>`Console::WriteLine(_T("Green"));`<br>`Console::SetTitle(_T("My App"));`<br>`Console::Clear();` |
...
| Standard Template Library (STL) | DotNetDupe |
| :--- | :--- |
| `// Manual parsing and comparison logic`<br>`struct Ver { int major, minor; ... };`<br>`bool operator<(const Ver& a, const Ver& b) { ... }` | `Version v1(_T("1.2.3.4"));`<br>`Version v2(1, 2, 4, 0);`<br>`if (v1 < v2) { ... }` |

### 8. File I/O (FileStream) 💾
| Standard Template Library (STL) | DotNetDupe |
| :--- | :--- |
| `std::fstream fs("data.bin", std::ios::in \| std::ios::out \| std::ios::binary);`<br>`fs.seekp(10, std::ios::beg);`<br>`fs.write(buffer, size);` | `FileStream fs(_T("data.bin"), FileMode::Open, FileAccess::ReadWrite);`<br>`fs.Seek(10, SeekOrigin::Begin);`<br>`fs.Write(buffer, 0, size);` |

### 9. Efficient String Building 🏗️
| Standard Template Library (STL) | DotNetDupe |
| :--- | :--- |
| `std::stringstream ss;`<br>`ss << "Items: " << 10 << ", Price: " << 19.99;`<br>`std::string s = ss.str();` | `StringBuilder sb;`<br>`sb.Append(_T("Items: ")).Append(10).Append(_T(", Price: ")).Append(19.99);`<br>`String s = sb.ToString();` |
...
| Standard Template Library (STL) | DotNetDupe |
| :--- | :--- |
| *(No standard equivalent; requires manual parsing)* | `UriBuilder ub(_T("https"), _T("github.com"));`<br>`ub.SetPath(_T("/google/gemini"));`<br>`String url = ub.ToString();` |

### 14. Type Conversion 🔄
| Standard Template Library (STL) | DotNetDupe |
| :--- | :--- |
| `try { int i = std::stoi("12345"); }`<br>`catch (const std::invalid_argument& e) { ... }` | `try { int i = Convert::ToInt32(_T("12345")); }`<br>`catch (const FormatException& e) { ... }` |

### 15. Environment & OS 💻
| Standard Template Library (STL) | DotNetDupe |
| :--- | :--- |
| `char* p = std::getenv("PATH");`<br>`// No standard OS version/platform API` | `String p = Environment::GetEnvironmentVariable(_T("PATH"));`<br>`OperatingSystem os = Environment::GetOperatingSystem();` |
