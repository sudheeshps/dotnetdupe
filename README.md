...
2.  **Build the solution and generate NuGet package:**
    ...
    .\BuildAndPack.ps1
    ```
    This script will update the resource build timestamp, compile the x64 and x86 Release binaries, and output the NuGet package (`DotNetDupe.4.0.5.nupkg`) into the `nuget_packages` directory.
...
.\BuildAndPack.ps1
```
This updates the build timestamp, compiles both x64 and x86 Release binaries, and outputs `DotNetDupe.4.0.5.nupkg` inside the `nuget_packages/` directory.
...
1. Extract `DotNetDupe.4.0.5.nupkg` (ZIP format) to a local directory:
   ```powershell
   Expand-Archive -Path "nuget_packages\DotNetDupe.4.0.5.nupkg" -DestinationPath "DotNetDupe_NuGet" -Force
   ```
