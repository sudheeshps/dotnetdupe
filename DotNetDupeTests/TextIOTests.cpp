...
namespace SystemTests {
    namespace TextIOTestCases {
        ...
        TEST(StringWriterTest, Write_Should_AppendText_When_Invoked) {
            ...
            ASSERT_EQ(sw.ToString(), "Hello World");
        }
        ...
        TEST(StringReaderTest, ReadLine_Should_ReturnLines_When_Invoked) {
            ...
            // When: Reading lines
            // Then:
            ASSERT_EQ(sr.ReadLine(), "Line 1");
            ASSERT_EQ(sr.ReadLine(), "Line 2");
            ASSERT_EQ(sr.ReadLine(), "Line 3");
        ...
        TEST(StringReaderTest, ReadToEnd_Should_ReturnAllRemainingText_When_Invoked) {
            ...
            String content("Some text to read");
            StringReader sr(content);
            // When: Reading one char then reading to end
            sr.Read();
            String remaining = sr.ReadToEnd();
            // Then:
            ASSERT_EQ(remaining, "ome text to read");
        }

        TEST(StringReaderTest, Peek_Should_ReturnNexcharWithoutConsuming_When_Invoked) {
            // Given: A StringReader
            StringReader sr("ABC");
            // When: Peeking
            int p1 = sr.Peek();
