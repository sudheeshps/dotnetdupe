/// \file Task.h
/// \brief Represents an asynchronous operation mirroring .NET System.Threading.Tasks.Task.
    ...
    namespace System {
        namespace Threading {
            namespace Tasks {
                ...
                ///
                /// Provides task scheduling on the ThreadPool, completion synchronization via ManualResetEvent,
                /// and status querying (Created, Running, RanToCompletion, Faulted, Canceled).
                /// Thread-safe for concurrent status queries and Wait() invocations.
                ///
                /// \note Conforms to the .NET Task-based Asynchronous Pattern (TAP).
                /// \see ThreadPool, Thread
                class Task : public Object {
                public:
                    /// \brief Initializes a new Task with the specified action delegate.
                    ...
                    /// \brief Starts the Task, scheduling it for execution to the ThreadPool.
                    DOTNETDUPE_API void Start();

                    /// \brief Waits indefinitely for the Task to complete execution.
                    ...
                    /// \return True if the task has completed (RanToCompletion, Faulted, or Canceled).
                    DOTNETDUPE_API bool GetIsCompleted() const;

                    /// \brief Gets whether the Task completed due to an unhandled exception.
                    /// \return True if the task threw an unhandled exception; otherwise, false.
                    ...
                    /// \brief Queues the specified work to run on the ThreadPool and returns a Task representing that work.
                    /// \param objAction The work delegate to execute asynchronously.
                    /// \return A SmartPointer to the running Task.
