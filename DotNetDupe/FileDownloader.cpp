        ...
        namespace Net {
            namespace Http {
                ...
                FileDownloader::FileDownloader(const String& sUrl, const String& sDestinationPath)
                    : m_sUrl(sUrl), m_sDestinationPath(sDestinationPath) {
                ...
                void FileDownloader::Pause() {
                    if (m_status.load() == DownloadStatus::Downloading) {
                        m_bPauseRequested = true;
                ...
                bool FileDownloader::Start() {
                    if (m_status.load() == DownloadStatus::Downloading) return false;
                    ...
                    Console::WriteLine(String("[FileDownloader] Starting download from ") + m_sUrl);

                    std::thread workerThread([this]() { this->DownloadLoop(); });
                    workerThread.detach();
                    return true;
                }

                bool FileDownloader::Resume() {
                    DownloadStatus currentStatus = m_status.load();
                    if (currentStatus == DownloadStatus::Downloading || currentStatus == DownloadStatus::Completed) return false;

                    m_bPauseRequested = false;
                    m_status = DownloadStatus::Downloading;
                    Console::WriteLine(String("[FileDownloader] Resuming download for ") + m_sDestinationPath);
                ...
                long long FileDownloader::CheckExistingFileSize() {
                ...
                long long FileDownloader::ParseContentLengthFromResponse(const SmartPointer<HttpResponseMessage>& pResponse) {
                ...
                void FileDownloader::AddHeaders(const Collections::Generic::Dictionary<String, String>& headers) {
                    for (auto const& [sKey, sVal] : headers) {
                        m_customHeaders[sKey] = sVal;
                    }
                }

                void FileDownloader::SetUserAgent(const String& sUserAgent) {
                    m_customHeaders["User-Agent"] = sUserAgent;
                }

                long long FileDownloader::FetchContentLength() {
                    try {
                        HttpClient client;
                        Console::WriteLine(String("[FileDownloader] Sending HEAD request to URL: ") + m_sUrl);
                        auto pRequest = SmartPointer<HttpRequestMessage>::NewShared(HttpMethod("HEAD"), Uri(m_sUrl));
                        ...
                        auto pResponse = client.Send(pRequest);
                ...
                SmartPointer<HttpRequestMessage> FileDownloader::CreateGetRequest(long long llRangeStart) {
                    auto pRequest = SmartPointer<HttpRequestMessage>::NewShared(HttpMethod::Get, Uri(m_sUrl));
                    pRequest->GetHeaders().Add("Accept", "*/*");
                    pRequest->GetHeaders().Add("User-Agent", "DotNetDupe-FileDownloader/1.0");
                ...
                void FileDownloader::UpdateRate(long long llBytesInSession, const std::chrono::steady_clock::time_point& timeStart) {
                ...
                bool FileDownloader::ReadAndWriteData(const SmartPointer<IO::Stream>& pStream, IO::FileStream& outFile) {
                    char pBuffer[8192];
                ...
                void FileDownloader::NotifyStateChange(DownloadStatus status) {
                    m_status = status;
                    m_dDownloadRate = 0.0;
                ...
                void FileDownloader::ExecuteDownload(long long llExistingBytes) {
                    auto pRequest = CreateGetRequest(llExistingBytes);
                    HttpClient client;
                    SmartPointer<HttpResponseMessage> pResponse;
                    Console::WriteLine(String("[FileDownloader] Requesting URL: ") + m_sUrl);
                    try {
                        pResponse = client.Send(pRequest, HttpCompletionOption::ResponseHeadersRead);
                    } catch (const HttpRequestException& ex) {
                ...
                void FileDownloader::DownloadLoop() {
